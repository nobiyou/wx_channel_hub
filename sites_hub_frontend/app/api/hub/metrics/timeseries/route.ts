import { NextRequest, NextResponse } from "next/server";

import { HubUnauthorizedError, hubFetch } from "@/lib/hub";
import { getOptionalHubToken } from "@/lib/session";

export async function GET(request: NextRequest) {
  const token = await getOptionalHubToken();
  if (!token) {
    return NextResponse.json({ error: "未登录" }, { status: 401 });
  }

  const range = request.nextUrl.searchParams.get("range") || "15m";

  try {
    const timeseries = await hubFetch(`/api/metrics/timeseries?range=${encodeURIComponent(range)}`, undefined, token);
    return NextResponse.json({ timeseries });
  } catch (error) {
    if (error instanceof HubUnauthorizedError) {
      return NextResponse.json({ error: "登录已失效" }, { status: 401 });
    }
    return NextResponse.json({ error: "读取时序数据失败" }, { status: 500 });
  }
}
