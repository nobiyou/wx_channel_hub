import { NextRequest, NextResponse } from "next/server";

import { HubUnauthorizedError, getBrowseRecords } from "@/lib/hub";
import { getOptionalHubToken } from "@/lib/session";

export async function GET(request: NextRequest) {
  const token = await getOptionalHubToken();
  if (!token) {
    return NextResponse.json({ error: "未登录" }, { status: 401 });
  }

  const machineId = request.nextUrl.searchParams.get("machine_id") || "";
  const page = Number(request.nextUrl.searchParams.get("page") || "1");
  const pageSize = Number(request.nextUrl.searchParams.get("page_size") || "20");

  try {
    const data = await getBrowseRecords({ machineId, page, pageSize }, token);
    return NextResponse.json({ data });
  } catch (error) {
    if (error instanceof HubUnauthorizedError) {
      return NextResponse.json({ error: "登录已失效" }, { status: 401 });
    }
    return NextResponse.json({ error: "读取浏览记录失败" }, { status: 500 });
  }
}
